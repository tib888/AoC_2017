...
fn task1(input: &str) -> usize {
    let re = Regex::new(r"^p=<([-\d]+),([-\d]+),([-\d]+)>, v=<([-\d]+),([-\d]+),\s*([-\d]+)>, a=<([-\d]+),([-\d]+),([-\d]+)>")
        .unwrap();
    ...
    let mut minindex = 0 as usize;

    for _ in 0..1000 {
        let mut mindist = <i64>::max_value();
        
        for i in 0..lines.len() {
            let mut l = &mut lines[i];
            l.v.x+=l.a.x;
            l.v.y+=l.a.y;
            l.v.z+=l.a.z;
            l.p.x+=l.v.x;
            l.p.y+=l.v.y;
            l.p.z+=l.v.z;
            let d = l.p.x.abs() + l.p.y.abs() + l.p.z.abs();
            if d < mindist {
                mindist = d;
                minindex = i;
            }
...
fn task2(input: &str) -> usize {
    ...
    for _ in 0..1000 {
        ...
        lines.sort_by(|a, b| a.p.cmp(&b.p));

        let mut d = 0;
        let mut s = 0;
        loop {
            let mut n = 1;
            while s + n < lines.len() && lines[s].p == lines[s+n].p {
                n += 1;
            }
            if n == 1 {
                lines[d] = lines[s];
...
fn main() {
    let d20 = "p=<1500,413,-535>, v=<-119,22,36>, a=<-5,-12,3>
